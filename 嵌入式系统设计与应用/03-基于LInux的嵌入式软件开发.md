...
- [3. 嵌入式内核结构及移植](#3-嵌入式内核结构及移植)
  ...
  - [3.3 Linux内核配置原理](#33-linux内核配置原理)
  - [3.4 Linux 内核编译](#34-linux-内核编译)
  - [3.5 内核模块的编程和普通程序的编程的区别](#35-内核模块的编程和普通程序的编程的区别)
  - [3.6 设备树](#36-设备树)
...
- [6. 设备驱动](#6-设备驱动)
  - [6.1 Linux驱动操作命令](#61-linux驱动操作命令)
  - [6.2 Linux驱动程序框架](#62-linux驱动程序框架)
...
- `配置工具`:配置后保存到`.config`的内核配置文档中
  - `make config`: 基于字符界面的配置界面
  - `make menuconfig`: 基于文本菜单的配置界面
  - `make xconfig`: 基于图形界面的用户配置界面

`Linux`配置时，每一项都有几种选项，分别为`Y`、`N`、`M`
  - `Y`: 代表将此项编译入内核中
  - `N`: 代表不将此项编译
...
- `make mrproper`：清除临时文件
- `make defconfig`：默认配置
- `make menuconfig`：进一步配置
- `make`：生成内核模块以及 vmlinuz, initrd.img, System.map 文件
- `make zImage`：生成内核镜像 zImage
- `make dtbs`：生成设备树

## 3.5 内核模块的编程和普通程序的编程的区别

因为内核模块运行在内核态，所以其包含的头文件都是内核程序相关的头文件，而普通程序则包含的是`glibc`的头文件。

模块程序没有`main`函数，而是通过`init_module`函数在加载后初始化。

内核模块程序内可直接调用内核函数，而普通程序则只能通过系统调用使用内核函数。

普通用户程序可运行多次，而内核函数通常则只能加载一次

## 3.6 设备树

设备树文档：

![image-20240102202502801](images/03-基于LInux的嵌入式软件开发/image-20240102202502801.png)

设备树的基本语法：

![image-20240102202614888](images/03-基于LInux的嵌入式软件开发/image-20240102202614888.png)
...
- `#address-cells` 和 `#size-cells`
  - `address-cells` 决定子节点 `reg` 属性中地址信息所占用的字长
  - `size-cells` 决定子节点 `reg` 属性中长度信息所占的字长
- `ranges`属性
  - 一个地址映射表，由子地址、父地址和地址空间长度三部分组成
- `phandle`
  - 值为 `<u32>` 类型，为设备树中唯一的节点指定一个数字标识符
  - `phandle`属性值用于需要引用与该属性关联的节点的其他节点

设备树的常用节点：

![image-20240102202644644](images/03-基于LInux的嵌入式软件开发/image-20240102202644644.png)
...
- `cat /proc/modules`：显示驱动模块大小、在内核空间中的地址
- `cat /proc/devices`：只显示驱动主设备号，且是分类显示
- `lsmod`：查看当前已经安装的驱动列表
- `insmod xxx_drv.ko`：加载设备驱动
- `rmmod xxx_drv.ko`：加载设备驱动
- `modprobe`：加载设备驱动，包括依赖模块驱动
- `ls -l /dev/xxx`：查看设备文件
- `mknod /dev/xxx c 252 0`：创建设备文件
- `rm /dev/xxx`：删除设备文件

## 6.2 Linux驱动程序框架

`file_operations` 结构体: 只要实现一个`file_operations`结构体并注册到内核中，内核就有了操作此设备的能力。

`inode` 结构体: 内核使用`imode`结构体在内核内部表示一个文件。

`inode`与`file`的区别：
- `file` 表示打开的文件描述符
- 可以使用多个 `file` 结构表示同一个文件的多个文件描述符，但所有的这些`file`结构全部都必须只能指向一个`inode`结构体。
...
- `dev_t i rdev`;
  ...
  - 该字段包舍了真正的设备编号
- `struct cdev *i_cdev`;
  - `struct_cdev` 是表示字符设备的内核的内部结构
  - 当 `inode` 指向一个字符设备文件时，该字段包含了指向 `struct cdev`结构的指针

---

一个Linux内核模块主要由以下几个部分组成:
- **模块加载函数** (必须)：执行`insmod`或`modprobe`命令加载内核模块时，模块的加载函数就会自动被内核执行，完成本模块的相关初始化工作。
- **模块卸载函数** (必须)：执行`rmmod`命令卸载模块时，模块卸载函数就会自动被内核执行，完成相关清理工作。
- **模块许可证声明** (必须)
- **模块参数** (可选)
- **模块导出符号** (可选)
- **模块作者等信息声明** (可选)
...
- `copy_from_user()`把数据从用户空间拷贝到内核空间
